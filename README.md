...
**AI Litigation Dashboard**의 기능을 효과적으로 활용하는 방법입니다.

1.  **Dataset 선택**: 상단 헤더의 `CSV Select`에서 데이터를 골라 `Update`를 클릭합니다.
2.  **지도 탐색**: `USA` / `WORLD` 간 전환, 마우스 휠을 이용한 줌/이동, 툴팁 확인이 가능합니다.
3.  **다중 필터 & 검색**: 헤더의 `진행상태`로 소송 단계를 거르고, 사이드바의 `Search`로 특정 사건을 찾습니다.
4.  **상세 정보 & 내보내기**: 개별 사건 클릭 시 상세 모달이 뜨며, 결과 통계를 `Export CSV`로 저장합니다.

👉 **[상세 사용자 가이드 (Detailed User Guide)](file:///work/github-aigovsensing/ai-suit-dashboard/doc/user_guide.md)** 에서 각 기능의 자세한 활용법을 확인하세요.
